...
//declare function
func swap(integers: inout [Double], firstIndex: Int, secondIndex: Int) {
...
while passswap != 0 {
    passswap = 0
    for index in 0 ..< unsortedIntegers.count {        
        if (index + 1 < unsortedIntegers.count) && (newarray[index] > newarray[index + 1]) {
            swap(integers: &newarray, firstIndex: index, secondIndex: index + 1)
            passswap += 1
            totalswap += 1
        }
    }
